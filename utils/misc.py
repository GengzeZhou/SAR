...
def init_sys_print(local_out_path, only_sync_master=False):
    if local_out_path is not None: os.makedirs(local_out_path, exist_ok=True)
    _change_builtin_print(dist.is_local_master())
    if (dist.is_master() if only_sync_master else dist.is_local_master()) and local_out_path is not None and len(local_out_path):
        sys.stdout, sys.stderr = SyncPrint(local_out_path, sync_stdout=True), SyncPrint(local_out_path, sync_stdout=False)
...
class MetricLogger(object):
    ...
    def log_every(self, start_it, max_iters, itrt, print_freq, header=None):
        ...
        # print(self.log_iters, 'x'*100)
        # self.log_iters.add(start_it)
        self.log_iters = print_freq
        if not header:
            header = ''
...
def auto_resume(args: arg_util.Args) -> Tuple[List[str], int, int, dict, dict]:
    info = []
    
    # Look in parent directory (experiment directory without timestamp)
    parent_dir = os.path.dirname(args.local_out_dir_path)
    
    if args.continue_training_ckpt is not None:
        ckpt_path = args.continue_training_ckpt
        info.append(f'[auto_resume] resuming from {ckpt_path} ...')
    elif args.resume_epoch is not None:
        # Look for specific epoch checkpoint
        specific_pattern = f'ar-ckpt-epo{args.resume_epoch}.pth'
        search_pattern = os.path.join(parent_dir, '*', specific_pattern)
        all_ckpt = glob.glob(search_pattern)
        
        if len(all_ckpt) == 0:
            info.append(f'[auto_resume] no ckpt found for epoch {args.resume_epoch} @ {search_pattern}')
            info.append(f'[auto_resume quit]')
            return info, 0, 0, {}, {}
        else:
            # Use the most recent checkpoint if multiple found
            ckpt_path = sorted(all_ckpt, key=os.path.getmtime, reverse=True)[0]
            info.append(f'[auto_resume] resuming from epoch {args.resume_epoch} @ {ckpt_path} ...')
    else:
        # Look for the latest checkpoint across all timestamp directories
        search_pattern = os.path.join(parent_dir, '*', 'ar-ckpt-*.pth')
        all_ckpt = glob_with_latest_modified_first(search_pattern)
        
        if len(all_ckpt) == 0:
            info.append(f'[auto_resume] no ckpt found @ {search_pattern}')
            info.append(f'[auto_resume quit]')
            return info, 0, 0, {}, {}
        else:
            ckpt_path = all_ckpt[0]
            info.append(f'[auto_resume] resuming from latest ckpt @ {ckpt_path} ...')
    ...
    # If epoch not in checkpoint, try to parse from filename (e.g., FlexVARd24-epo349.pth)
    if ep == 0:
        import re
        filename = os.path.basename(ckpt_path)
        match = re.search(r'-epo(\d+)\.pth', filename)
        if match:
            ep = int(match.group(1))
            info.append(f'[auto_resume] parsed epoch {ep} from checkpoint filename')

    resume_trainer = ckpt.get('trainer', None)
    resume_args = ckpt.get('args', None)
    info.append(f'[auto_resume success] resume from ep{ep}, it{it}')
    return info, ep, it, resume_trainer, resume_args
