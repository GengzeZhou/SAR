...
class Args(Tap):
    data_path: str = '/path/to/imagenet/ILSVRC2012'
    exp_name: str = 'text'
    ...
    vae_ckpt: str = "pretrained/FlexVAE.pth"  # path to vqvae
    vae_v: int = 8912    # codebook size for VQVAE (common values: 8192)
    vae_c: int = 32      # VAE channels (embedding dimension)

    continue_training_ckpt = None
    mixed_precision="bf16"
    num_workers = 32
    ckpt_every = 5000
    resume_epoch: Optional[int] = None  # specify which epoch checkpoint to resume from
    ...
    # ScaleAR specific
    model_type: str = 'flexvar'   # 'flexvar' or 'scalear' 
    randar_scale: int = 4         # starting scale for RandAR generation (e.g., 4 means 4x4)
    randar_mode: str = 'maskgit'  # 'randar' (with position instruction tokens) or 'maskgit' (original MAR-style)
    enable_var_kv: bool = False   # Enable KV caching for VAR scales during inference
    mask_ratio_min: float = 0.7   # minimum masking ratio for RandAR/MaskGIT training (truncated Gaussian)
    share_randar_var_pos_embed: bool = False  # Share positional embeddings between RandAR and VAR steps
    training_mode: str = 'teacher_forcing'  # 'teacher_forcing' or 'student_forcing' or 'mixed' or 'alternating' or 'hybrid_tf_sf' or 'csfl' or 'tf_then_single_sf' for ScaleAR training
    sigma: float = 0.5            # mixing weight for mixed training mode (teacher_loss * sigma + student_loss * (1 - sigma))
    hybrid_tf_scales: int = 8     # number of scales for hybrid training mode
    
    # Student Forcing CFG and Sampling Parameters
    sf_cfg_scale: float = 1.2     # CFG scale for student forcing sampling (1.0 = no CFG, >1.0 = enable CFG)
    sf_top_k: int = 900           # Top-k sampling for student forcing (0 = disabled)
    sf_top_p: float = 0.96        # Top-p sampling for student forcing (0.0 = disabled)  
    sf_use_sampling: bool = True  # Whether to use sampling vs argmax for student forcing
    ...
    save_epo: int = 5       # saving model ckpt
    vis_num_samples: int = 32   # number of samples to generate for visualization
    vis_cfg_scale: float = 1.2  # classifier-free guidance scale for visualization
    vis_save: bool = True       # whether to save visualization during training
    vis_intermediate_scales: bool = False  # whether to visualize intermediate scales (4x4, 8x8, etc.)
    vis_cosine_steps: int = 8   # number of cosine steps for ScaleAR inference
    ...
    logger_type: str = 'tensorboard'   # 'tensorboard' or 'wandb'
    wandb_project: str = 'sar'         # wandb project name
    wandb_entity: str = None           # wandb entity/team name (optional)
    wandb_run_name: str = None         # wandb run name (optional, will be auto-generated if not provided)
    wandb_tags: str = None             # comma-separated tags for wandb run
    wandb_notes: str = None            # notes for wandb run
    wandb_api_key: str = None          # wandb API key (optional, can also use WANDB_API_KEY env var)
    wandb_host: str = None             # custom wandb host (optional)
    ...
    # environment
    local_out_dir_path: str = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'out')  # [automatically set; don't specify this]
    ...
    overfit_debug: bool = False  # overfit on first 4 images from dataset for debugging
...
def init_dist_and_get_args():
    ...
    dist.broadcast(timestamp_tensor, src_rank=0)# Broadcast the timestamp to all processes
    timestamp = int(timestamp_tensor.item())# All processes receive the timestamp
    dist.barrier()
    formatted_date = '{}{}-{}-{}-{}:{}:{}'.format(*([f"{timestamp}"[i:i+2] for i in range(0, 14, 2)]))
    
    # experiment_index = len(glob(f"{args.local_out_dir_path}/VARd{args.depth}-{args.exp_name}/*"))
    # args.local_out_dir_path = os.path.join(args.local_out_dir_path, f'VARd{args.depth}-{args.exp_name}', f'Exp-{experiment_index:03d}')
    args.local_out_dir_path = os.path.join(args.local_out_dir_path, f'{args.exp_name}-d{args.depth}', f'{formatted_date}')
    os.makedirs(args.local_out_dir_path, exist_ok=True)
    misc.init_sys_print(local_out_path=args.local_out_dir_path)
    ...
    tb_name = _reg_valid_name.sub(
        '_',
        f'tb-VARd{args.depth}-{args.exp_name}'
        # f'__pn{args.pn}'
