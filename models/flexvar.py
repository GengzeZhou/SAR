...
class FlexVAR(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        return_intermediate_scales=False, cosine_steps=None,
    ...
    def forward(self, label_B: torch.LongTensor, x_BLCv_wo_first_l: torch.Tensor, infer_patch_nums, student_forcing: bool = False) -> torch.Tensor:  # returns logits_BLV
        """
        :param label_B: label_B
        :param x_BLCv_wo_first_l: teacher forcing input (B, self.L-self.first_l, self.Cvae)
        :param infer_patch_nums: patch numbers for this training step
        :param student_forcing: if True, use student forcing mode (prepend class token only, no SOS)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if student_forcing:
                # Student forcing mode: prepend class token only (no SOS for scales)
                x_embed = self.word_embed(x_BLCv_wo_first_l.to(cond_BD.dtype))
                x_BLC = torch.cat([cond_BD.unsqueeze(1), x_embed], dim=1)
            else:
                # Teacher forcing mode: use SOS for first scale
        ...
        if student_forcing:
            # For student forcing: skip class token (index 0), return logits for actual tokens only
            logits = self.get_logits(x_BLC[:, 1:], cond_BD)
        else:
            # For teacher forcing: return logits for all tokens
            logits = self.get_logits(x_BLC, cond_BD)

        return logits    # logits BLV, V is vocab_size
